...
class report_graphic extends Gcharts {
  ...
  /**
   * Create a chart of events most taken course including non-grading ones.
   * @return string the google charts data.
   */
  public function get_courses_usage() {
    ...
    $sql = "SELECT distinct course, scormid, userid, fullname as coursename, count(*) totalaccess
      ...
      ON c.id=s.course
      INNER JOIN mdl_scorm_scoes_track t
      ON s.id = t.scormid
      WHERE
      ...
      AND t.value IN ('completed', 'passed', 'failed', 'complete', 'incomplete')
      )
      AND
      c.id > 1
      AND t.timemodified
    ...
    // Format the data to google charts.
    $i = 1;
    $courseusage[0] = array(get_string('course'), get_string('quantity', 'report_graphic'));
    foreach ($result as $coursedata) {
      $courseusage[$i] = array($coursedata->coursename, (int)$coursedata->totalaccess);
      $i++;
    }
